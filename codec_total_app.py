...
class BatcherNameApp:
    ...
    def __init__(self, root):
        ...
        # Radiobutton para manejo del sufijo: "keep" (por defecto) o "delete"
    ...
    def create_widgets(self):
        ...
        tk.Checkbutton(main_frame,
                       ...
                       variable=self.enumerated_prefix_var).pack(pady=10, anchor="w")
    ...
    def rename_advance(self, original, new_name_val, prefix_val, suffix_val, enumerated_prefix, enumeration_number):
        ...
        if mode == "replace_after":
            ...
            # Usamos rsplit para separar la parte anterior del separador (última ocurrencia)
            parts = base.rsplit(sep, 1)
            
            #Fixed this part, for error change the all name.
            if len(parts) == 2:
                left_part, right_part = parts
                orig_suffix = sep + right_part  # SE CORRIGE: se conserva siempre la parte derecha
            else:
                left_part = base
                orig_suffix = ""
            new_text = new_name_val if new_name_val else (f"{prefix_val}{enumeration_number:02d}" if prefix_val and enumerated_prefix 
                                                          else (prefix_val if prefix_val else left_part))
            if suffix_val:
                new_suffix = suffix_val if suffix_val.startswith(sep) else sep + suffix_val
            else:
                new_suffix = orig_suffix if self.suffix_mode.get() == "keep" else ""
